...
func NewUDSDispatcher(logger *zap.Logger, cfg DispatcherConfig) EventDispatcher {
	d := &udsDispatcher{
		logger:   logger,
		config:   cfg,
...
func (d *udsDispatcher) Enqueue(data []byte) error {
...
func (d *udsDispatcher) sender() {
	...
	for batch := range d.outbound {
		...
		if conn == nil {
			var err error
			conn, err = net.DialTimeout("unix", d.config.TargetAddr, 3*time.Second)
			if err != nil {
				d.logger.Error("uds dial failed", zap.Error(err))
				d.releaseBatch(batch)
				time.Sleep(500 * time.Millisecond)
				continue
			} else if unixConn, ok := conn.(*net.UnixConn); ok {
				_ = unixConn.SetWriteBuffer(16 * 1024 * 1024)
				_ = unixConn.SetReadBuffer(16 * 1024 * 1024)
			}
		}
		...
		if err != nil {
			d.logger.Error("uds transport error", zap.Error(err))
			cleanup()
		}
...
func (d *udsDispatcher) releaseBatch(batch [][]byte) {
	for i := range batch {
		if batch[i] != nil {
